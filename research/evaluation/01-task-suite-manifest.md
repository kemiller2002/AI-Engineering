...
| ET-004 | Update one bounded document while preserving front matter and unrelated edits | Medium | Write | Requested semantic change; clean diff; no collateral change | State + deterministic | Pilot calibrated; independent review pending |
...
| ET-014 | Detect that an evaluation task itself is underspecified or contradictory | Medium | Read | Defect classified with evidence; no fabricated resolution | Human + rubric | Pilot calibrated; independent review pending |
...
Obtain independent reviews of ET-004-P01 and ET-014-P01, then export blind fixtures and run the first repeated baseline. Do not expose `outcomes/` or `expected-calibration.json` to the evaluated agent.
