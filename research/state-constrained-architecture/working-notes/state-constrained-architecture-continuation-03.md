...
Healthcare
    suspected -> supported -> established -> contradicted

Finance
    pending -> authorized -> settled -> reversed

Insurance
    filed -> reviewed -> approved/denied -> paid

DevOps
    built -> tested -> approved -> deployed

Security
    requested -> authenticated -> authorized -> revoked

Research
    hypothesis -> supported -> contradicted -> verified

Legal / Compliance
    draft -> reviewed -> approved -> effective

Manufacturing
    queued -> assembled -> inspected -> released

Logistics
    created -> packed -> shipped -> delivered -> returned

Customer Support
    opened -> triaged -> investigating -> resolved

HR
    candidate -> interviewed -> offered -> hired
...
HelixNote:
    Claim + Evidence + Epistemic State + Provenance

Clarity:
    Decision State + Evidence + Capability + Obligation

DevOps:
    State + Capability + Invariant + Effect
...
The test looked specifically for concepts that would force the introduction
of fundamentally new primitives.
...
Capability:
    Clinician may establish diagnosis
...
The main wrinkle is that medical state is often knowledge ABOUT reality,
not reality itself.
...
Coordinator:
    Prevent mutually incompatible concurrent operations

Obligation:
    Reconcile unknown payment outcome
...
This was important because research is not primarily transactional,
yet the same architecture still works.
...
This suggests the architecture is more like a semantic grammar than
a mandatory one-size-fits-all object model.
...
It is not yet clear whether Subject deserves to be a canonical primitive
or is simply identity attached to other concepts.


PART 2 — Boundary Testing: Where NOT to Use It
==============================================

The architecture should not be treated as a universal replacement
...
There is little meaningful lifecycle, authority, evidence,
...
The correctness properties are mathematical,
...
Without obligations, the architecture can block unsafe actions
but still fail to ensure important unresolved conditions are addressed.
...
3. Monitoring
    ...
    RecheckFraudStatusBeforeShipping
...
Priority should ideally derive from policy,
not arbitrary numeric ordering.
...
When policy cannot decide the conflict,
...
Creating hundreds of thousands of heavyweight workflows immediately
may be wasteful.
...
Once obligations and capabilities are both explicit,
the architecture can derive paths from:
...
An obligation should provide a goal,
not necessarily a hard-coded procedure.
...
The latter may indicate a broken specification
and should potentially be detected at compile time.
...
The same specification that prevents illegal actions
can also generate legal plans.
...
The next test asked whether one declarative specification
could drive all of:
...
If documentation, runtime behavior, tests, and agent tools
all originate from the same transition definition,
they cannot drift independently as easily.
...
It takes consequential domain rules and produces
the deterministic environment within which probabilistic agents operate.
...
Find the smallest semantic vocabulary that can express
the tested domains without turning into a general-purpose programming language.
...
An agent may be allowed to propose a diagnosis
without possessing the final establishment authority.
...
Because the DSL is intentionally constrained,
the compiler can ask questions ordinary application code cannot easily answer.
...
Agents may invoke Tier 2 only through permissions/capabilities
...
Agents may propose specification changes,
but that is itself a governed operation.
...
This prevents agents from solving blocked tasks
by simply weakening the rules.
...
By sacrificing computational expressiveness,
...
A semantic compiler for agent-operated software could provide
a deterministic world model for probabilistic agents.
...
If those three substantially different workflows fit cleanly,
the idea begins to look less like an architectural philosophy
and more like an implementable semantic platform/compiler.
